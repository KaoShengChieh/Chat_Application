...
public class Client implements Runnable {
	...
	public Client(String clientAddress, BlockingQueue<Packet> sendQueue, BlockingQueue<Packet> recvQueue, ClientHandler clientHandler, List<ClientHandler> clientList, Statement stmt) {
	...
	private void recvFile(Message recv_msg) throws SQLException {
		try {
			Runtime rt = Runtime.getRuntime();
			Process pr = rt.exec("java FileReceiver " + recv_msg.content + " " + clientAddress);
		} catch (IOException e) {
			//sendQueue.push(new Packet(Packet.Type.FILE, "Unable to receive file: " + e.getMessage()));
		}
	}
}
